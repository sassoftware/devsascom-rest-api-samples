...
To see how to navigate to a caslib, see the Data Sources API documentation.

#### <a name='example-get-tables'>Retrieving Tables</a>

The entry point into this service must be from a data source's `tables` link to retrieve a collection of
`application/vnd.sas.data.table` resources.
For example, perform a GET request on the `application/vnd.sas.data.source` resource below:
...
resource that contains `application/vnd.sas.data.table` resources.
...
The `application/vnd.sas.data.table` resource includes a `columns` link. Use this link to request an
`application/vnd.sas.collection` resource that contains `application/vnd.sas.data.column.summary` resources.
...
#### Retrieving Row and Row Set Data

The `application/vnd.sas.data.table` resource includes `rows` and `rowSet` links that you can follow to request either
`application/vnd.sas.collection` of
`application/vnd.sas.data.row` or a `application/vnd.sas.data.row.set` resource, respectively.
Use the `rows` link to get the full row of data for the table, regardless of the number of columns. Use the `rowSet` link to get rows of data for specific columns or a range of columns.

**Rows and rowSet Link from the application/vnd.sas.data.table**
...
`application/vnd.sas.data.provider` and check the value of the `usesSessions` member.
...
This `session` link must be the `application/vnd.sas.data.session` [media type](http://developer.sas.com/reference/Media_type "Media type"), and the href must be `/dataSources/providers/{providerId}/sources/{sourceId}/sessions/{sessionId}`.
...
##### Use Case 1: Client Provides sessionId</a>
...
##### Use Case 2: Client Does Not Specify the sessionId
...
##### Use Case 3: Client Does Not Specify the sessionId and specifies preserveSession=true
...
for a given source data table that is defined by the `tableUri` member of the `sourceArguments` object. For more information, see [available request media types](#media-types.md).
...
If additional information is needed by the `application/vnd.sas.data.table.job+json` resource, use the `alternate` link to access the backing `application/vnd.sas.job.execution.job+json`.
...
below. If the `delimiter` is not specified, the default is `,`. For more information, see [media types](#media-types.md).
...
version 3, last updated 22 May, 2019