...
Retrieves the application/vnd.sas.api+json representation
...
Uses the POST method to submit a job request to the service.
...
A separate operation is available to submit a job to conceptualize documents in the request.

The response code is 202 (Accepted for processing), and the response body is a JSON
representation of the job containing HATEOAS links.
...
Uses the POST method to submit a job request to the service.
The request body contains the configuration information of the job, 
including where to store the input data and the text documents to conceptualize.
The `Content-Type` header is `application/vnd.sas.text.concepts.job.request.documents+json`.
A separate operation is available that submits a job to conceptualize documents in a table.
...
Returns a paginated list of all the jobs in the repository. Optionally, these jobs can be filtered and sorted.

The collection contains up to __limit__ items.

The collection links include the pagination links `prev`, `first`, `next`, `last`,
...
Here is an example of getting job information for a specific job ID,
using the `/jobs/{jobId}` path. This example uses the job ID `"aa43f983-ab7a-420e-972a-4a66e3e2ee5b"` to get the job information.
The request asks for the resource by using
the representation `application/vnd.sas.text.concepts.job+json`.
The response is a JSON object. 

The `cancel` link is included if the job can
...
Returns the current state of a job for a specific job ID. Valid values are:
...
Returns the log of a job for a given job ID, where each line in the log is separated by one line break (`\n`).
The response body is `text/plain` and the status code is 200 (Request succeeded).
...
Returns a paginated list of all of the lines of the log for a specified job ID.

The collection contains up to __limit__ items.

The collection links include pagination links `prev`, `first`, `next`, `last`,
if those pages are available from the current page.
...
Returns any errors that occurred while the job was executing.

This endpoint returns 204 (No content) if no errors have occurred.
Otherwise, the endpoint returns 200 (Request succeeded) and the response body is
...
Fetches the links to the results for a given job, if the links are available. This endpoint 
is able to fetch results only when the job state is `completed`. Otherwise, the endpoint returns
404 (Results do not exist). If enabled, links to the fact results are present.
...
Fetches the concept results as a collection for a specified job, if any are available. This endpoint
is able to fetch concept results only when the job state is `completed`. Otherwise, this endpoint returns
a 404 (No results exist).
...
Fetches the fact results as a collection for a specified job, if results are available. This endpoint
is able to fetch fact results only when the job state is `completed`. Otherwise, the endpoint returns
404 (No results exist).
...
Deletes the job resource for the specified job ID.
...
version 2, last updated 22 May, 2019