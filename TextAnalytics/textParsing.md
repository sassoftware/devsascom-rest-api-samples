...
* [GeGet the Log of a Specific Job Formatted as a Collection](#GetFormattedCollectionJobLog)
...
This call gets the application/vnd.sas.api+json representation
of the APIs top-level links.
...
This call uses the POST method to submit a job request to the service.
The request body contains the configuration information for the job, including where the input data is stored.
The `Content-Type` header is `application/vnd.sas.text.parsing.job.request+json`.
A separate operation is available to submit a job that parses documents that are loaded directly to the service.

If the request is successful, the response code is 202 (Accepted for processing), and the response body is a JSON
representation of the job with HATEOAS links.
The API representation contains `self`, `delete`, `state`, `log`, and `caslib` links. The `cancel` link is included if the job can
be canceled. Similarly, the `results` link is included if the job has completed successfully,
...
This call uses the POST method to submit a job request to the service.
The request body contains the configuration information for the job,
including where to store the input data and the text documents to parse.
The `Content-Type` header is `application/vnd.sas.text.parsing.job.request.documents+json`.
A separate operation is available to submit a job that parses documents in a table.
...
### <a name='GetInfoAllJobs'>Get All Jobs</a>

This call gets a paginated list of all jobs in the repository, optionally filtered and sorted.
The collection contains up to __limit__ items.
The collection links include pagination links `prev`, `first`, `next`, and `last`
if those pages are available from the current page.
...
### <a name='GetInfoByJobID'>Get a Job by jobId</a>

To look up a job resource for a specific job ID,
use the `/jobs/{jobId}` path. This example gets the job
resource with job ID `"f0250e89-5475-4de5-bddf-1f78b3a3a4f9"`.
The request asks for the resource using
the representation `application/vnd.sas.text.parsing.job+json`.
The response is a JSON object. The `cancel` link is included if the job can
be canceled. Similarly, the `results` link is included if the job
has completed successfully, and its results are available.
...
This call gets the current state of a job for a specified job ID. Valid values are:
 - pending
 - running
 - completed
 - failed
...
If the request is successful, the response body is `text/plain` and the status code is 200 (The request succeeded).

```
running
```

### <a name='GetJobLogText'>Get the Log of a Specific Job as Text</a>

This call retrieves the log of a job for a specified job ID, where each line is separated by one line break (`\n`).
If the request is successful, the response body is `text/plain` and the status code is 200 (The request succeeded).
If no log is available, then an empty response is returned, and the `Content-Length` header has a value of `0`.
...
### <a name='GetFormattedCollectionJobLog'>Get the Log of a Specific Job Formatted as a Collection</a>

Get a paginated list of all of the lines in the log from a job with a specified job ID.
The collection contains up to __limit__ items.
The collection links include pagination links `prev`, `first`, `next`, and `last`,
if those pages are available from the current page.
...
### <a name='GetJobErrors'>Get the Errors from a Specific Job</a>

This call retrieves any errors that occurred when the job was executing.
This endpoint returns 204 (No content) if no errors occurred.
Otherwise, the call returns 200 (The request succeeded), and the response body is
an `application/vnd.sas.error+json;version=2` JSON payload.
...
### <a name='GetJobResults'>Get the Results from a Specific Job</a>

This call fetches the results of a specified job ID, if any are available. This endpoint
is able to fetch results only when the job state is `completed`. Otherwise, the endpoint returns code
404 (No results exist at the requested path).
...
This call deletes the job with the specified job ID.
Invoking this endpoint deletes the job and all related resources.
If the call is successful, the response body is empty and the status code is 204 (No content).

version 3, last updated 07 May, 2019
