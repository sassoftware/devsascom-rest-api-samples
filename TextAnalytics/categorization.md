...
* [Discover Top-Level Links for Categorization API]()
</details>
...
### <a name=''>Discover Top-Level Links for Categorization API</a>
...
A separate operation is available to submit a job to categorize documents in the request.
...
The API representation contains `self`, `delete`, `state`, `log` and `caslib` links. The `cancel` is included if the job can
...
including where to store the input data as well as the text documents to categorize. 
The `Content-Type` header is `application/vnd.sas.text.categorization.job.request.documents+json`.
A separate operation is available that submits a job to categorize documents in a table.
...
Get a paginated list of all jobs in the repository. (Optional) You can filter and sort the list.
The collection contains up to __limit__ items.
The collection links include the pagination links, `prev`, `first`, `next` and `last`,
...
resource with job ID `"4dd4f43e-abc2-4ecc-9391-341a334562f9"`.
The request asks for the resource using
the representation `application/vnd.sas.text.categorization.job+json`.
...
### <a name=''>Get the State of a Specific Job</a>

This call returns the current state of a job for a job ID that you specify. Valid values are:
...
This call retrieves the log of a job for a given job ID, where each line is separated by one line break (`\n`).
The response body is `text/plain` and the status code is 200 (Request succeeded).
...
Get a paginated list of all of the lines of the log for a given job ID.
The collection contains up to __limit__ items.
...
### <a name=''>Get the Errors for a Specific Job</a>

This call retrieves any errors that occurred while the job was executing.
This endpoint might return 204 (No content) if no errors have occurred.
Otherwise, it returns 200 (Request succeeded) and the response body is
...
### <a name='GetJobState'>Get the Results for a Specific Job</a>

This call retrieves the results for a given job, if they are available. This endpoint
is able to fetch categorization results only when the job state is `completed`, otherwise it returns
...
This call deletes the job resource for the specified job ID.
Invoking this endpoint deletes the job and all related resources.
If the request to delete is successful, the response body is empty and the
